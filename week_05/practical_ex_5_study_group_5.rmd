...
pacman::p_load(lme4, tidyverse, readbulk, boot, interactions)
```
...
```{r}
#loading data 
data <- read_bulk(directory = "/Users/antondrasbaek/Desktop/github_methods_3/week_05/experiment_1")

unique(data$seed) #NA's are already there (perhaps due to readbulk)
...
Our log_likelihood_function does not exactly match the logLik function when applied to a partial pooling model. This is expected considering that the likelihood is calculated differently for multilevel models which is not taken into account in the formula used in our function. 
...
LINK TO REPORT: https://docs.google.com/document/d/1H3XydA8hSaq9Km6MN8T78q_r6Cr46UNR67R3tC5NFwc/edit
...
```{r}
interact_plot(model = m6, pred = "target.frames", modx = "pas") 
```
